...
class DoubleMassPendulumFitter:
    ...
    def plot_fit(self, popt, pcov=None, show_confidence=True):
        ...
        ax1.set_xlabel('Расстояние до нижнего груза R_н (мм)', fontsize=12)
        ax1.set_ylabel('Период T (с)', fontsize=12)
        ax1.set_title('Маятник с двумя грузами\n(верхний на 43 мм выше оси)', fontsize=14)
        ...
        ax2.errorbar(self.R_lower_vals * 1000, T2_vals, 
                    yerr=T2_err_vals, fmt='o', capsize=5,
                    label='Экспериментальные данные', color='green', markersize=8)
        
        T2_fine = self.model_double_mass(R_fine, I0_fit, m_fit)
        ax2.plot(R_fine * 1000, T2_fine, 'r-', linewidth=2, label='Модель')
        ax2.set_xlabel('Расстояние до нижнего груза R_н (мм)', fontsize=12)
...
def analyze_double_mass_pendulum():
    ...
    R_lower_vals = np.array([0.253, 0.211, 0.185, 0.165, 0.145, 0.125, 0.105])
    T_vals = np.array([
    ...
    ax.set_xlabel('Расстояние до нижнего груза R_н (мм)', fontsize=12)
    ax.set_ylabel('Период T (с)', fontsize=12)
    ax.set_title('Сравнение моделей маятника', fontsize=14)
